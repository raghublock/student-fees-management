...
function StudentProfile() {
  ...
  // 🚀 NAYA JAADOO: Month-Wise Grouping Logic
  let monthlyPassbook = [];
  if (!student.has_active_plan && paymentHistory.length > 0) {
      const ledgerMap = {};
      
      paymentHistory.forEach(item => {
          const mStr = item.month || 'Other';
          const monthsArr = mStr.split(',').map(m => m.trim());
          // Agar advance payment multiple months ka hai, toh equally divide karo
          ...
          monthsArr.forEach(m => {
              ...
              // Agar system ne bill generate kiya hai (Laal rang)
              if (item.status === 'Billed') {
                  ledgerMap[m].billed += amountPerMonth;
              } 
              // Agar user ne paise jama kiye hain (Hara rang)
              else {
      ...
      // Dictionary ko Array mein convert karke, Naye mahine ko sabse upar laana
      monthlyPassbook = Object.values(ledgerMap).sort((a, b) => new Date(b.monthName) - new Date(a.monthName));
  }
      ...
      <div className="print:hidden">
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className={`${student.has_active_plan ? 'bg-orange-500' : 'bg-slate-800'} p-4 text-white font-black uppercase tracking-widest text-center text-sm`}>
                {student.has_active_plan ? `📦 ACTIVE ${config.planLabel.toUpperCase()} PURCHASE RECORD` : '📜 MONTHLY STATEMENT & PASSBOOK'}
            </div>
            <table className="w-full text-center border-collapse">
                <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 tracking-widest">
                <tr>
                    <th className="p-4 border-b text-left pl-6">Billing Month</th>
                    <th className="p-4 border-b text-slate-500">Total Fee (Billed)</th>
                    <th className="p-4 border-b text-green-600">Amount Paid</th>
                    <th className="p-4 border-b text-right pr-6">Month's Balance</th>
                </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                {student.has_active_plan ? (
                     // PRO Students ke liye wahi purani table chalegi (unka monthly summary nahi hota)
                     paymentHistory.length > 0 ? paymentHistory.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50 transition-all border-b">
                            <td className="p-4 font-bold text-slate-500 uppercase text-xs text-left pl-6">{item.start_date}</td>
                            <td className="p-4 text-xs font-black text-slate-700">{item.plan_name}</td>
                            <td className="p-4 font-black text-green-600 text-sm">₹{item.price}</td>
                            <td className="p-4 font-black text-xs text-right pr-6">✅ Cleared</td>
                        </tr>
                     )) : <tr><td colSpan="4" className="p-10 text-slate-300 font-black italic uppercase text-xs">No plan history found.</td></tr>
                ) : (
                    // 🚀 REGULAR STUDENTS ke liye naya Month-Wise Summary logic
                    monthlyPassbook.length > 0 ? monthlyPassbook.map((row, idx) => {
                        const monthDue = row.billed - row.paid; // Us mahine ka hisaab nikalna
                        ...
                        <tr key={idx} className="hover:bg-slate-50 transition-all border-b">
                            ...
                            <td className="p-4 text-left pl-6">
                                <span className="font-black text-indigo-900 uppercase text-sm">
                                    🗓️ {row.monthName}
                                </span>
                            </td>
                            ...
                            {/* Column 3: Total Paid */}
                            <td className="p-4 font-black text-green-600 text-sm bg-green-50/30">
                                ₹{row.paid}
                            </td>

                            {/* Column 4: Month Balance */}
                        ...
                        <tr><td colSpan="4" className="p-20 text-slate-300 font-black italic uppercase text-xs tracking-widest">No history found.</td></tr>
                    )
                )}
                </tbody>
            </table>
