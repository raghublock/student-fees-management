  ...
  const handlePurchase = async (e) => {
    ...
    try {
      ...
      if(res.ok) {
        ...
        toast.error("Plan activate nahi hua.", { id: loadingToast });
      }
    } catch (err) {
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen font-sans">
      ...
      <div className="max-w-7xl mx-auto bg-white p-8 rounded-2xl shadow-xl mb-10 border-t-8 border-orange-500">
        <h2 className="text-lg font-black mb-6 text-slate-700 uppercase">Naya Plan Chunein</h2>
        <form onSubmit={handlePurchase} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex flex-col gap-1">
            ...
            <select className="border-2 p-3 rounded-xl focus:border-orange-500 outline-none font-bold text-slate-700" value={formData.student_id} onChange={(e) => setFormData({...formData, student_id: e.target.value})} required>
              ...
              {students.map(s => <option key={s.id} value={s.id}>{s.name} {s.has_active_plan ? '(Already PRO)' : ''}</option>)}
            </select>
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Plan Name (Shift)</label>
            <input type="text" placeholder="e.g. Full Day / Morning Shift" className="border-2 p-3 rounded-xl focus:border-orange-500 outline-none font-bold" value={formData.plan_name} onChange={(e) => setFormData({...formData, plan_name: e.target.value})} required />
          </div>

          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Duration (Months)</label>
            <input type="number" placeholder="Months (e.g. 1)" className="border-2 p-3 rounded-xl focus:border-orange-500 outline-none font-bold" value={formData.duration_months} onChange={(e) => setFormData({...formData, duration_months: e.target.value})} required />
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input type="date" className="border-2 p-3 rounded-xl focus:border-orange-500 outline-none font-bold" value={formData.start_date} onChange={(e) => setFormData({...formData, start_date: e.target.value})} required />
          </div>
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-100">
        <div className="bg-slate-800 p-4">
          <h3 className="text-white font-black uppercase tracking-wider">Active Subscription History 📜</h3>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {plans.length > 0 ? plans.map(p => (
                <tr key={p.id} className="hover:bg-orange-50/50 transition-colors">
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <span className="font-black text-slate-800 uppercase">{p.student_name}</span>
                      <span className="bg-orange-100 text-orange-600 text-[8px] px-2 py-0.5 rounded-full font-bold">PRO</span>
                    </div>
                  </td>
                  <td className="p-4">
                    <span className="font-bold text-slate-600">{p.plan_name}</span>
                    <span className="text-xs text-slate-400 block">{p.duration_months} Month Subscription</span>
                  </td>
                  <td className="p-4">
                    <span className="text-red-500 font-black px-3 py-1 bg-red-50 rounded-lg text-xs border border-red-100 uppercase">
                      ⌛ {p.expiry_date}
                    </span>
                  </td>
                  <td className="p-4">
                    <span className="text-green-700 font-black text-lg italic">₹{p.price}</span>
                  </td>
                  <td className="p-4 text-center">
                    <button 
                      onClick={() => handleDeletePlan(p.id)} 
                      className="bg-red-100 text-red-600 px-4 py-2 rounded-xl font-black text-[10px] hover:bg-red-600 hover:text-white transition-all shadow-sm uppercase border border-red-200"
                    >
                      Delete
                    </button>
                  </td>
                ...
                <tr>
                  <td colSpan="5" className="p-10 text-center text-slate-400 font-bold italic uppercase">Abhi tak koi active plan record nahi mila hai.</td>
                </tr>
              )}
            </tbody>
          </table>
